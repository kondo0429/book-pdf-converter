...
| Output page size | Output keeps the source's pixel dimensions rather than the original's fixed 3508-px-high pages: each processed page is fitted back to its extracted source page (PDF, embedded at the extraction DPI so the physical page size is also preserved) or input file (JPEG). With page-area cropping the page is **enlarged** (aspect preserved) until it matches the source width or height, and the axis that overshoots is cut back. Cropping to the sheet leaves an aspect ratio taller and narrower than the camera frame, so fitting the page *inside* the source size would shrink it and band the sides with white; scaling by the larger of the two ratios fills the frame with no padding. The overshoot spends the slack the page crop carries beyond the print - mostly the blank below the page number. Where the window starts is one figure for the whole book, not each page's own ink: a page carrying less ink would otherwise shift its window and print its page number at a different height from the facing page. It is one figure for both parities too, since the windows already sit at the same height against the paper - a per-parity figure would line up the *tops* of the print instead, and a running head on one side only would then push the page numbers apart by that much. Without page-area cropping the page carries no such slack (the crop is already text plus even margins, and enlarging would trim into the running head), so it is fitted inside the source size and centred on white instead |
