...
# 2-/3-color printed books (black + red, black + red + blue, ...): keep the ink
# colors while still removing show-through and background (all pages, or only some)
book-pdf-converter input.pdf output.pdf --bleed-keep-color
book-pdf-converter input.pdf output.pdf --bleed-keep-color-pages 10-24
...
Options:
  ...
  --bleed-keep-color    Show-through removal: keep ink colors instead of
                        grayscale output (2-/3-color printed books)
  --bleed-keep-color-pages PAGES
                        Page numbers (1-indexed) to keep ink colors in
                        show-through removal, e.g. "1,4,7-9"
  --no-margin-whitening
...
| Show-through & background removal | On by default: instead of the original's global-linear color adjustment, each page's paper background is estimated locally (morphological closing + blur), the image is flat-field normalized so paper becomes uniformly white, and a contrast stretch (`--bleed-black-point` / `--bleed-white-point`) decides what is content and what is background. The harsh stretch is not used as the final rendering (it would clip glyph fringes and thin the strokes): pixels it keeps dark form an importance mask, the mask is slightly dilated, and inside it gently stretched flat-field tones are composited over a pure-white background - white background and smooth anti-aliased text at the same time. Removes reverse-side ghost text AND non-uniform background color; output is grayscale. For 2-/3-color printed books, `--bleed-keep-color` (all pages) / `--bleed-keep-color-pages` (listed pages) keep the ink colors: each RGB channel is flat-fielded and stretched on its own, so the paper still becomes uniform white while red/blue ink keeps its hue; content is decided on the darkest channel (colored ink can be light in luminance but is dark in at least one channel). Wide flat color areas (a band bleeding off the page edge, a tinted title bar) would otherwise be taken for paper background and flattened to white, and margin whitening would paint the rest, so colored areas are detected from paper-white-balanced chroma (a pale tint that sits right at the threshold pixel by pixel is also taken whole from a smoothed low-resolution chroma map, so it does not come out mottled with white holes), left out of the background estimate (normalized instead by the paper lighting carried in from the surrounding paper, so the band keeps an even tone where the lighting falls off toward the page edge) and protected from margin whitening (only inside the detected sheet, so the colored edges of the page stack are not kept). Sheet detection also measures brightness as the max RGB channel, so a color band at the page edge is not mistaken for the stand. Exclude color/photo pages with `--bleed-removal-exclude-pages` (they keep the original color adjustment) or disable with `--no-bleed-removal` |
| Output page size | Output keeps the source's pixel dimensions rather than the original's fixed 3508-px-high pages: each processed page is fitted back to its extracted source page (PDF, embedded at the extraction DPI so the physical page size is also preserved) or input file (JPEG). With page-area cropping the page is **enlarged** (aspect preserved) until it matches the source width or height, and the axis that overshoots is cut back. Cropping to the sheet leaves an aspect ratio taller and narrower than the camera frame, so fitting the page *inside* the source size would shrink it and band the sides with white; scaling by the larger of the two ratios fills the frame with no padding. The overshoot spends the slack the page crop carries beyond the print - mostly the blank below the page number. Filling the frame with the crop *window* wastes whatever of it is margin rather than print (about a quarter of the width here) and leaves the print wherever it happens to sit, which came out pressed against the top; the scale is therefore what makes the **print** fill the frame down to an even margin (3% of the frame's shorter side), and the frame is centred on the print. Both come from one set of print bounds for the whole book, so this is still a single scale factor and the character size stays identical on every page - taking them from each page's own ink would move the frame by however much ink that page happened to carry, and the page number would then sit at a different height from the facing page. The scale is fitted to the 85th percentile of the per-page print size rather than the largest: one figure page whose print already fills the frame would otherwise hold the whole book at its current size, and the few longer pages instead spend their margin. The bounds are medians, since the typical page is the one that should come out evenly margined. Without page-area cropping the page carries no such slack (the crop is already text plus even margins, and enlarging would trim into the running head), so it is fitted inside the source size and centred on white instead |
...
| Geometry sidecar | `--geometry FILE` records the geometry a run decided - every page's crop window, the shared crop and output size, the window placement, the OCR page-number alignment and each page's text extent - as JSON. A `--no-bleed-removal` run reads that file back and reuses those decisions instead of deriving its own, and skips the OCR pass. Show-through removal is what flattens the paper to white, and that is what makes the sheet, the text extent and the page number reliably measurable; on a page whose paper was left as scanned, the same detections land differently, so a colour pass would frame its pages and place its page numbers differently from the grayscale pass. Record the geometry on a normal run, then pass the same file to the `--no-bleed-removal` run and the two outputs line up (verified to within 1 px). Without `--no-bleed-removal` the file is written, not read, so a normal run always refreshes it; if it is missing or unreadable the run falls back to deciding its own geometry |
| Page-area cropping | For book-scanner shots (the sheet photographed on a dark stand), the crop is cut from the **sheet** rather than from the text. The paper is located on the pre-adjustment image as the one large bright region, filtered by column/row coverage so the gutter dip and the connected sliver of the facing page fall away. Every page then gets the **same crop size**, which pins the scale and keeps the character size identical across the book; only the window's position follows each page's own sheet, absorbing the drift as the book's thickness shifts it across the shoot. Horizontally the window is hung off the fore-edge — the one clean paper-to-stand boundary, identified as the vertical page edge that does *not* sit on the frame border (the sheet runs past the frame on the gutter side, so that edge is just where the photo stops). Vertically one robust top per parity is used, since the rig does not move. Sizing the crop from the text bbox instead makes the frame depend on how much text a page carries, so a chapter opener ends up with a very different margin layout from a full page. This only engages when nearly every page shows a sheet on a clearly darker surround of plausible size; a borderless scan (already cropped to the paper, e.g. extracted from a PDF) has no such surround and keeps the text-bbox crop below, as does any book where the detections disagree enough to yield an implausibly small window |
| Group crop region | Used when page-area cropping does not engage. Odd and even pages each get a shared crop rectangle. The original uses the **median** of the per-page text bounding boxes (after Tukey-fence outlier rejection, k=1.5); this places the right edge at the median text extent, so any page whose text reaches the common (gutter-side) maximum has its outermost vertical column sliced off — the "right-edge characters cut" case. This port keeps the same outlier rejection (figure/junk-page bboxes are still dropped) but takes the **envelope** (min left, min top, max right, max bottom) of the remaining inliers instead. A shared crop must contain every page's text to avoid clipping any of it, so the correct boundary is the inlier extreme, not the median; no inlier page loses text. The envelope always contains the median rectangle, so nothing that survived before can newly disappear. The per-page text bounding box feeding this also differs: the C# detector blanks a 1% border before measuring (edge-noise guard), which caps the measurable extent ~50px inside the image and hides text that legitimately runs close to the page edge (gutter-side body text) — its outermost column would then be sliced by the crop. Since the box is measured after margin whitening has already cleaned the margins, only a 2px border is blanked (the full 1% is kept with `--no-margin-whitening`) |
