...
| 出力ページサイズ | 本家の高さ3508px固定ではなく、元のピクセルサイズを維持します：処理後のページを、抽出した元ページ（PDF。抽出DPIで埋め込むため物理ページサイズも一致）または入力ファイル（JPEG）のサイズに合わせます。紙面基準のクロップ時は、元画像の**幅または高さに一致するまで等比拡大**し、はみ出した側を切り戻します。紙面でクロップすると縦横比が撮影フレームより縦長・幅狭になるため、元サイズの内側にフィットさせると縮小されて左右に白帯が入ります。2つの比率の大きい方で拡大することで余白なしでフレームを埋めます。切り取られるのは紙面クロップが印刷領域より余分に持っている余白（主にページ番号より下の空白）です。窓の開始位置は書籍全体で1つの値を使います（ページごとのインク量から決めると、インクの少ないページで窓がずれ、対向ページとページ番号の高さが変わってしまいます）。奇数・偶数でも同じ値を使います — 両者の窓はすでに紙面に対して同じ高さにあるためで、パリティごとに分けると印刷領域の**上端**が揃ってしまい、片側だけにある柱の分だけページ番号が離れてしまいます。紙面基準のクロップが使えない場合は、そのような余りがない（本文＋均等余白ですでに詰まっており、拡大すると柱が欠ける）ため、従来どおり元サイズの内側にフィットさせて白地に中央配置します |
