...
| 余白の白色化 | デフォルトで有効：外周の余白バンド（最も左の文字より左、最も右の文字より右、最も上の文字より上、最も下の文字より下）を白に塗ります。ページ端に接し、反対側の端まで文字が一切ない帯だけが削除対象になるため、文字が誤って消されることはありません（背表紙の影やページ端の筋は除去されます）。文字と行・列を共有する領域には触れません。バンドを妨げてはならないスキャンごみは色調整前の画像で検出し、文字判定から除外します：画像の縁に接する暗領域（撮影台・スキャナ台）、細く連続した横バー（上下端の線）、細く連続した縦バー（左右のページ端・背表紙/ノドの影。薄い閾値で捕捉し、インク量ガードで実テキスト列は除外）。加えて、バンド適用後に左右マージンで2つの処理を行います：(a) なめらかで低コントラストな綴じ部の影はフラットフィールド補正で広がったレンズ状の染み（暗い核と薄い筋を持つ）として残りバンドを押し戻すため、外側マージンの暗い塊のうち「鋭い画素の割合が小さいもの」を白色化します（実テキストは鋭いストロークが大半、なめらかな染みはそうではない）。(b) 小口（ページ山）の影は暗くザラついたもの・非常に薄いもの・真っ黒な芯を持つものまでありますが、いずれも文字範囲の端を影の位置に固定してしまうため、余白バンドは影の手前で止まります。そこで「平滑化した列の明るさが沈んだ連続帯」を、白色化直後のバンド境界（文字範囲の端）から探索し、全高にわたって白色化します（フラットフィールド済みページの背景は正確に白なので、非常に薄い帯でも検出できます）。塗る前に帯がごみであることを検証します：文字には作れない「連続した非白の縦ラン」を持つ列が帯内に必要で、実際は本文だった帯は棄却されます。塗った帯の中でも、グリフサイズのインク成分（ページ番号・柱）は保護され、縦に長いインクの芯は除去されます。どちらの処理も鋭い実テキストには触れないため、端に届く見出し・本文・ページ番号・柱が切れることはありません。いずれも印刷物ではあり得ない構造のため安全で、写真には影響しません。白色化した帯の幅（各辺の中央値）は最終クロップの周囲に余白として再付与されるため（このモードでは `--margin-percent` の代わりに使用）、元のページと同程度の余白が保たれます。上下の余白は奇数・偶数ページで共通の値を使うため、文字の開始高さが揃います。`--no-margin-whitening` / `--margin-pad` |
