...
# 2色刷り・3色刷りの本（黒＋赤、黒＋赤＋青など）: 裏映り・背景除去を行いつつ
# インクの色を保持（全ページ、または指定ページのみ）
...
オプション:
  ...
  --bleed-white-point N 裏映り除去: この値以上を紙/白に。下げるほど強力
                        （デフォルト: 205）
  --bleed-keep-color    裏映り除去: グレースケールにせずインクの色を保持
                        （2色刷り・3色刷りの本向け）
  --bleed-keep-color-pages PAGES
                        裏映り除去でインクの色を保持するページ番号（1始まり）
                        例: "1,4,7-9"（それ以外のページはグレースケール）
  --no-margin-whitening
...
| 裏映り・背景除去 | デフォルトで有効：本家のグローバル線形色調整の代わりに、各ページの紙背景を局所推定（モルフォロジークロージング＋ブラー）し、フラットフィールド正規化で紙を均一な白に平坦化、さらにコントラストストレッチ（`--bleed-black-point` / `--bleed-white-point`）で内容と背景を判別します。ハードなストレッチは最終描画には使いません（文字のフリンジが削れて細くなるため）：ストレッチで暗く残る画素を重要部分マスクとし、少し膨張させたマスクの内側にフラットフィールド補正の滑らかな階調を合成、外側は純白にします。背景の白と文字のアンチエイリアスの滑らかさを両立します。裏面のゴースト文字と紙の色ムラの両方が消えます（出力はグレースケール）。2色刷り・3色刷りの本では `--bleed-keep-color`（全ページ）/ `--bleed-keep-color-pages`（指定ページ）でインクの色を保持できます：RGB各チャンネルを個別にフラットフィールド正規化・ストレッチするため紙は均一な白になり、赤や青のインクは色相を保ちます。内容の判定には最も暗いチャンネルを使います（色インクは輝度が高くても、いずれかのチャンネルでは暗いため）。ページ端まで届く色帯や色付きの見出し帯のような広い色面は、そのままでは紙の背景とみなされて白く平坦化され、残りも余白の白色化で塗られてしまうため、紙色で白バランスを取った彩度から色面を検出し（画素単位ではしきい値付近にある淡い色帯も、平滑化した低解像度の彩度マップから一続きの面として取り込むため、白い穴でまだらになりません）、背景推定から除外（周囲の紙から補間した照明で正規化するため、ページ端に向かって暗くなる照明でも色帯の濃さが均一に保たれます）したうえで余白の白色化でも保護します（紙面の外、たとえばページ山の色付きの小口は対象外）。紙面検出も輝度ではなくRGBの最大値で行い、ページ端の色帯を撮影台と誤認して紙面から外すことを防ぎます。カラー/写真ページは `--bleed-removal-exclude-pages` で除外（本家の色調整が適用）、`--no-bleed-removal` で全体無効化 |
| 出力ページサイズ | 本家の高さ3508px固定ではなく、元のピクセルサイズを維持します：処理後のページを、抽出した元ページ（PDF。抽出DPIで埋め込むため物理ページサイズも一致）または入力ファイル（JPEG）のサイズに合わせます。紙面基準のクロップ時は、元画像の**幅または高さに一致するまで等比拡大**し、はみ出した側を切り戻します。紙面でクロップすると縦横比が撮影フレームより縦長・幅狭になるため、元サイズの内側にフィットさせると縮小されて左右に白帯が入ります。2つの比率の大きい方で拡大することで余白なしでフレームを埋めます。切り取られるのは紙面クロップが印刷領域より余分に持っている余白（主にページ番号より下の空白）です。クロップ**窓**でフレームを埋めると、窓のうち印刷ではない余白部分（この例では幅の約4分の1）が無駄になり、印刷領域も成り行きの位置（実際には上端に寄っていました）になります。そこで倍率は「**印刷領域**が均等余白（フレーム短辺の3%）までフレームを埋める」ように決め、フレームを印刷領域の中央に合わせます。いずれも書籍全体で1組の印刷領域境界から算出するため、倍率は単一のままで文字の大きさは全ページ同一です（ページごとのインクから決めると、インク量の違いでフレームがずれ、対向ページとページ番号の高さが変わってしまいます）。倍率の基準は最長ページではなく**85パーセンタイル**とします — 印刷が既にフレームを埋めている図版ページ1枚のために書籍全体を現状の大きさに据え置くことになるためで、それより長い少数のページは自身の余白を使います。境界には中央値を使います（均等な余白になるべきは典型ページのため）。紙面基準のクロップが使えない場合は、そのような余りがない（本文＋均等余白ですでに詰まっており、拡大すると柱が欠ける）ため、従来どおり元サイズの内側にフィットさせて白地に中央配置します |
...
| ジオメトリ情報ファイル | `--geometry FILE` で、その実行が決定したジオメトリ（各ページのクロップ窓、共通のクロップ寸法と出力サイズ、窓の配置位置、OCRによるページ番号の位置合わせ、各ページの本文範囲）をJSONで記録します。`--no-bleed-removal` での実行はこのファイルを読み戻して同じ決定を再利用し、OCRもスキップします。紙面・本文範囲・ページ番号を安定して測れるのは、裏映り除去が紙を白に平坦化しているからです。裏映り除去を行わないページでは同じ検出が違う結果になるため、カラーのまま出力すると枠取りやページ番号の高さがグレースケール版とずれてしまいます。通常実行でジオメトリを記録し、同じファイルを `--no-bleed-removal` の実行に渡すことで、両者の出力が一致します（実測±1px以内）。`--no-bleed-removal` を付けない場合はファイルを読まずに書き出すため、通常実行では常に最新化されます。ファイルが無い・読めない場合は自前でジオメトリを決定します |
| 紙面基準のクロップ | ブックスキャナ撮影（暗い撮影台の上に紙面を置いて撮影したもの）では、本文ではなく**紙面**を基準にクロップします。紙面は色調整前の画像で「1つの大きな明るい領域」として検出し、列・行の被覆率でフィルタすることでノドの落ち込みや繋がってしまった対向ページの端を除去します。その上で**全ページ共通のクロップ寸法**を使うため倍率が固定され、本全体で文字の大きさが一致します。位置だけは各ページの紙面に追従するので、本の厚み変化による紙面のドリフトを吸収します。水平方向は**小口**（紙と台の境界がはっきり出る唯一の辺。フレーム端に接していない方の垂直辺として判定します — ノド側は紙面がフレームからはみ出すため、その辺は「写真が途切れた位置」でしかありません）に合わせ、垂直方向は撮影装置が動かないためパリティごとの代表値を1つ使います。本文の外接矩形を基準にすると、そのページの文字量で枠が決まってしまい、章扉と本文ページで余白の付き方が大きく変わります。この方式は「ほぼ全ページで、十分に暗い背景に囲まれた妥当な大きさの紙面」が検出できた場合のみ有効になります。すでに紙面で切り出された画像（PDFから抽出したスキャンなど）にはそのような背景がないため下記の本文基準クロップが使われ、検出が食い違って極端に小さい枠になる場合も同様にフォールバックします |
| グループクロップ領域 | 紙面基準のクロップが使えない場合に使用します。奇数・偶数ページごとに共通のクロップ矩形を決めます。本家は外れ値（Tukeyフェンス、k=1.5）を除いた各ページ本文枠の**中央値**を矩形にしますが、これだと本文がノド（綴じ側）の共通右端まで達するページで最右列が中央値の外に落ち、縦書きの端の文字が切られます。本移植版は外れ値除去（図版・ゴミページの本文枠は従来通り除外）はそのままに、残った正常ページの本文枠の**外接矩形**（最小左・最小上・最大右・最大下）を採用します。共通クロップは全ページの本文を包含する必要があるため、境界は中央値ではなく正常ページの最外郭が正しく、これにより正常ページの文字が一切切られません（外接矩形は常に中央値矩形を包含するので、従来残っていた内容が新たに消えることはありません）。この基になる各ページの本文枠検出も変更しています：本家の検出器は計測前に外周1%を白で塗りつぶす（端ノイズ対策）ため、計測可能な範囲が画像端から約50px内側までに制限され、ページ端近くまで達する本文（ノド側の本文列）が計測から漏れてクロップに切られてしまいます。本文枠は余白白色化（端ごみ除去済み）の後に計測されるため、塗りつぶしは2pxのみにしています（`--no-margin-whitening`時は本家通り1%） |
