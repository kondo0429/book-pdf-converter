...
| 裏映り・背景除去 | デフォルトで有効：本家のグローバル線形色調整の代わりに、各ページの紙背景を局所推定（モルフォロジークロージング＋ブラー）し、フラットフィールド正規化で紙を均一な白に平坦化、さらにコントラストストレッチ（`--bleed-black-point` / `--bleed-white-point`）で内容と背景を判別します。ハードなストレッチは最終描画には使いません（文字のフリンジが削れて細くなるため）：ストレッチで暗く残る画素を重要部分マスクとし、少し膨張させたマスクの内側にフラットフィールド補正の滑らかな階調を合成、外側は純白にします。背景の白と文字のアンチエイリアスの滑らかさを両立します。裏面のゴースト文字と紙の色ムラの両方が消えます（出力はグレースケール）。2色刷り・3色刷りの本では `--bleed-keep-color`（全ページ）/ `--bleed-keep-color-pages`（指定ページ）でインクの色を保持できます：RGB各チャンネルを個別にフラットフィールド正規化・ストレッチするため紙は均一な白になり、赤や青のインクは色相を保ちます。内容の判定には最も暗いチャンネルを使います（色インクは輝度が高くても、いずれかのチャンネルでは暗いため）。カラー/写真ページは `--bleed-removal-exclude-pages` で除外（本家の色調整が適用）、`--no-bleed-removal` で全体無効化 |
